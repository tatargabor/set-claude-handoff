...
- [ ] 9.1 **Worktree blindness (measured):** a fleet-held agent working a worktree writes its handoff into the WORKTREE's `.set/handoff/`; a `/clear` reinject resolving from the process cwd (main repo) never sees it and loads a different thread's handoff by mtime. Live case: clear of the consumer's `the fleet-held agent` reloaded `0912-a16a` (main repo) while the thread's real state was `0912-ff48` in the consumer worktree — bridged manually via the owner-write load instruction. Fix candidates: the reinject (and gate) enumerate `git worktree list` handoff dirs, or resolve the dir from the transcript's actual working paths rather than cwd only.
- [ ] 9.2 **Owner-write executor for fleet-held TUI agents (measured):** ownerd holds interactive `claude` TUIs (no `-p`) — `OwnerClient.write(label, …)` delivered Escape + `/clear\r` into a live agent and the full chain ran (clear → fresh session id `dcc59baa` → reinject fired). The watcher's tmux-only writer skips exactly these (`no tmux pane hosts pid`, seen live for two ELIGIBLE sessions); adding an owner-client writer branch to `watch-auto-clear.sh` covers the fleet-held plane. Manual bridge used meanwhile: owner-write load instruction + queued task.
- [ ] 9.3 **Token file is per-project, not per-session (seen in the first armed pass):** two sessions in one repo share `.set/handoff/.context-tokens` — the statusline renders of either overwrite it, so session B can read session A's 474k+ as its own. Safe today only because the own-marker gate still blocks (B needs ITS handoff), but it is wrong; fix = write per-session files (`.context-tokens-<session8>` — the statusline input carries `session_id`).
