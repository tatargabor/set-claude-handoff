...
export function buildInjection({ dir, source, now = Date.now() }) {
  if (source !== "clear" && source !== "compact") return null // per-event allowlist

  if (!existsSync(dir)) return source === "clear"
  ...
  // Manual handoffs (not the machine `--allapot.md` pages), newest first.
  const manual = readdirSync(dir)
  ...
  if (manual.length > 0) {
    ...
    const p = join(dir, latest.f)
    let body = ""
    try { body = readFileSync(p, "utf8") } catch { body = null }
    parts.push([
      `## ⚠ Fresh context — handoff reloaded (${source})`,
      "",
      `**Latest handoff: \`${latest.f}\`** (chosen as newest by mtime — in a parallel-session tree, verify it is YOURS).`,
      manual.length > 1 ? `Other live handoffs: ${manual.slice(1, 6).map((k) => `\`${k.f}\``).join(", ")}.` : "",
      "",
      body == null
    ...
    parts.push([
      ...
      "",
      "No handoff exists in this repository. If a work thread was open before the clear, it was not written down; proceed carefully and re-measure state from the repo.",
      "",
    ].join("\n"))
    used += parts[0].length
  }
...
function main() {
  ...
  const injection = buildInjection({ dir, source: ev.source, now: Date.now() })
  if (!injection) return
